  ...
  for (const evt of events) {
    ...
    try {
      if (queryId === btcUSD) {
        ...
        label = 'BRL / USD';
        const response = await axios.get('https://v6.exchangerate-api.com/v6/REDACTED/latest/BRL');
        cgPrice = response.data.conversion_rates.USD;
      } else if (queryId === cnyUSD) {
        label = 'CNY / USD';
        const response = await axios.get('https://v6.exchangerate-api.com/v6/REDACTED/latest/CNY');
        cgPrice = response.data.conversion_rates.USD;
